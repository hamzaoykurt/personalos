...
export function TopBar({ title, menu, search, capture }: { title: string; menu: () => void; search: () => void; capture: () => void }) {
  return <header className="os-topbar"><button onClick={menu} aria-label="Menüyü aç"><Menu /></button><strong>{title}</strong><div><button className="os-search-button" onClick={search} aria-label="Her yerde ara" title="Her yerde ara"><Search /><span>Her yerde ara</span><kbd>⌘K</kbd></button><button className="os-top-capture" onClick={capture}><Plus /><span>Yeni kayıt</span></button></div></header>;
}
...
export function MobileNavigation({ active, go, capture }: { active: Section; go: (section: Section) => void; capture: () => void }) {
  ...
  return <nav className="os-mobile-nav" aria-label="Mobil navigasyon">{items.slice(0,2).map((item) => { const Icon = item.icon; return <button className={active === item.id ? "is-active" : ""} key={item.id} onClick={() => go(item.id)}><Icon /><span>{item.label}</span></button>; })}<button className="os-mobile-capture" onClick={capture} aria-label="Yeni kayıt"><i><Plus /></i></button>{items.slice(2).map((item) => { const Icon = item.icon; return <button className={active === item.id ? "is-active" : ""} key={item.id} onClick={() => go(item.id)}><Icon /><span>{item.label}</span></button>; })}</nav>;
}
